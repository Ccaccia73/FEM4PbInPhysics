...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  for(unsigned int b = 0; b <= basisFunctionOrder; ++b){
    if(b != node){
      double tmp = 1.;
      for(unsigned int c = 0; c <= basisFunctionOrder; ++c){
        if(c != node && c != b){
          tmp *= ( xi - xi_at_node(c) );
        }
      }
  ...
  // compute denominator of gradient expression
  double denom = 1.;
  for(unsigned int b = 0; b <= basisFunctionOrder; ++b){
  	if(b != node){
  	  denom *= ( xi_at_node(node) - xi_at_node(b) );
  	}
  }
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
      //EDITED - add component A of Flocal to the correct location in F
      ...
      for(unsigned int B=0; B<dofs_per_elem; B++){
        //EDITED - add component A,B of Klocal to the correct location in K (using local_dof_indices)
